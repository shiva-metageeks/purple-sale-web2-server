...
import SubscriptionData from "../../../model/chain/LaunchpadsInfo/subscriptionModel.js";
...
export const fetchDataAllArbitrum = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const chain = req.params.chain;
      const allData = await SubscriptionData.find({ chain });
...
export const fetchDataByIdArbitrum = catchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const chain = req.params.chain;
      const data = await SubscriptionData.findOne({ id: idParam, chain });
      if (!data) {
